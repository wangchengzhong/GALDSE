...
@BackboneRegistry.register("TinyNCSNpp")
class TinyNCSNpp(nn.Module):
    """NCSN++ model"""
        ...
        **kwargs):
        ...
        for i_level in reversed(range(num_resolutions)):
            ...
            if progressive != 'none':
                if i_level == num_resolutions - 1:
                    if progressive == 'output_skip':
                        modules.append(nn.GroupNorm(num_groups=min(in_ch // 4, 16), 
                            num_channels=in_ch, eps=1e-6))
                        modules.append(conv3x3(in_ch, self.total_channels, init_scale=init_scale))
                        pyramid_ch = self.total_channels
                    elif progressive == 'residual':
                        modules.append(nn.GroupNorm(num_groups=min(in_ch // 4, 16), num_channels=in_ch, eps=1e-6))
    ...
    def forward(self, x, time_cond=None):
        ...
        if self.conditional:
            ...
            temb = None
        # reverse the guidance for a little bit better performance
        x[:,[4],:,:] = 1 - x[:,[4],:,:]
...
if __name__ == "__main__":
    # python -m galdse.backbones.TinyNCSNpp
    model = TinyNCSNpp().eval()
    ...
    def input_constructor(input_shape):
        batch = 1
        x = torch.ones(()).new_empty((batch, *input_shape))
        time_cond = torch.ones((batch,), dtype=torch.float32)
